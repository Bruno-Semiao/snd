...
parser.add_argument("-refc", "--refChannel", dest="refChannel", help="reference channel for US TDC plots", default=4)
parser.add_argument("-correct", "--correctTime", dest="correctTime", help="correct timing measurements with regards to input channel", default=False)
...
if options.refChannel != 4:
  options.refChannel = int(options.refChannel)
if options.correctTime != False:
  try:
    options.correctTime = int(options.correctTime)
  except:
    options.correctTime = False
    print("correctTime set to False by default since argument is not an available channel.")
# MuFilter mapping of planes and bars. In the TI18 geometry plane 7 corresponds to the nonexistent horizontal layer of the 4th plane of the Downstream Muon System, while 8 is the vertical.
systemAndPlanes  = {1:2,2:5,3:7}
if options.geoFile:
  if "TI18" in options.geoFile: systemAndPlanes[3] = 8
...
if eventChain.GetBranch("Digi_MuFilterHit"):
  eventChain.Branch("Digi_MuFilterHits",eventChain.GetBranch("Digi_MuFilterHit"))
  print("Created (or attempted to) a Digi_MuFilterHit branch")
  #eventChain.Scan("Digi_MuFilterHits:Digi_MuFilterHit")
  #eventChain.GetBranch("Digi_MuFilterHits").Print()
  #eventChain.GetBranch("Digi_MuFilterHit").Print()
...
def Mufi_Efficiency(Nev=options.nEvents,optionTrack=options.trackType,c0=options.refChannel,NbinsRes=200,X=10.,correctTime=options.correctTime):

 projs = {1:'Y',0:'X'}
 correctRun = 46
 if correctTime:
   correction_file_path = "/afs/cern.ch/user/g/gmachadosnd/private/SNDBUILD/TDC_Cal/Run"+str(correctRun)+"/NewValuesRef"+str(correctTime)+".csv"
   if not os.path.exists(correction_file_path):
     print("No NewValuesRef file for channel "+str(correctTime)+" for Run " +str(correctRun) + ".")
     return -1
   correction_file = open(correction_file_path)
   csvreader = csv.reader(correction_file)
   header = next(csvreader)
   timeDict = {}
   for row in csvreader:
     #Tag is of the type ['Side','US2Plane','Bar-cChannel']
     #We are trying to save a dictionary with the shape:
     #timeDict[Plane][Side][Bar][Channel]=Mean
     Tag = row[0].split('_')
     if Tag[1] not in timeDict:
       timeDict[Tag[1]]={}
     if Tag[0] not in timeDict[Tag[1]]:
       timeDict[Tag[1]][Tag[0]] = {}
     if Tag[2][0] not in timeDict[Tag[1]][Tag[0]]:
       timeDict[Tag[1]][Tag[0]][Tag[2][0]] = {}
     if Tag[2][3] not in timeDict[Tag[1]][Tag[0]][Tag[2][0]]:
       timeDict[Tag[1]][Tag[0]][Tag[2][0]][Tag[2][3]] = 9999
     for index, feature in enumerate(header):
       if feature == "Mean":
         timeDict[Tag[1]][Tag[0]][Tag[2][0]][Tag[2][3]]=float(row[index])
        ...
        for bar in range(systemAndBars[s]):
             ...
             if (side=="") or (side=="S"):
                ...
                for i in range(systemAndChannels[s][1]+systemAndChannels[s][0]):
                        ...
                        ut.bookHist(h,'TDCcalib'+side+'_'+key+'-c'+str(i),'timing deviation of TDC ;dt [ns]',200,-10.0,10.0)
                        ut.bookHist(h,'TDCcalib'+side+'_'+key+'-c'+str(i)+"_sym",'symmetric timing deviation of TDC ;dt [ns]',200,-10.0,10.0)
...
# QDC/TDC channel variations
                  ...
                  t0Left   = 999
                  t0Right = 999
                  # c0 = options.refChannel == -1 implies that we want the median
                  if c0 == -1:
                      Left = numpy.empty(0)
                      Right = numpy.empty(0)
                      for i in allTDCs:
                        ...
                        if int(i) < 8:
                          Left=numpy.append(Left,allTDCs[i])
                        else:
                          Right=numpy.append(Right,allTDCs[i])
                      t0Left  = numpy.median(Left)
                      t0Right = numpy.median(Right)
                  else:
                    for i in allTDCs:
                      if i==c0: t0Left   = allTDCs[i]
                      if i==8+c0: t0Right = allTDCs[i]

                  for i in allTDCs:
                      if s==2 and smallSiPMchannel(i): continue
                      ######################
                      # Still don't have the deviation implemented for this
                      ######################
                  ...
                  for i in allTDCs:
                     ...
                     if (correctTime == -1) and (c0 != -1):
                          correfL = timeDict['US2'+str(plane)]['L'][str(bar)][str(c0)]
                          correfR = timeDict['US2'+str(plane)]['R'][str(bar)][str(c0)]
                     else:
                          correfL = 0
                          correfR = 0
                     ...
                     if i<nSiPMs and nL>0:
                          ...
                          if (i == c0) or (not correctTime):
                            corcha = 0
                          else:
                            corcha = timeDict['US2'+str(plane)]['L'][str(bar)][str(i)]
                          rc = h['sigmaTDCL_'+key].Fill( (allTDCs[i]-corcha-meanL/nL)*TDC2ns )
                          if t0Left<900: rc = h['TDCcalibL_'+key].Fill( (allTDCs[i] - t0Left)*TDC2ns -corcha+correfL)
                          if t0Left<900: rc = h['TDCcalibL_'+key+"_sym"].Fill( -(allTDCs[i] - t0Left)*TDC2ns +corcha-correfL)
                     elif not(i<nSiPMs) and nR>0:

                          if (i-8 == c0) or  (not correctTime):
                            corcha = 0
                          else:
                            corcha = timeDict['US2'+str(plane)]['R'][str(bar)][str(i-8)]
                          key =  sdict[s]+str(s*10+plane)+'_'+str(bar)+'-c'+str(i-nSiPMs)
                          rc = h['sigmaTDCR_'+key].Fill((allTDCs[i]-meanR/nR)*TDC2ns )
                          if t0Right<900: rc = h['TDCcalibR_'+key].Fill( (allTDCs[i] - t0Right)*TDC2ns -corcha+correfR)
                          if t0Right<900: rc = h['TDCcalibR_'+key+"_sym"].Fill( -(allTDCs[i] - t0Right)*TDC2ns +corcha-correfR)
 ...
 writing_file = '/afs/cern.ch/user/g/gmachadosnd/private/SNDBUILD/TDC_Histos/Channel'+str(c0)+'/MuFilterEff_run'+str(options.runNumber)+'.root'
 if correctTime:
   writing_file = writing_file[:-5] + "_cor" + str(correctTime) + ".root"
   print(writing_file)

 ut.writeHists(h,writing_file)
 #ut.writeHists(h,'/afs/cern.ch/user/g/gmachadosnd/private/SNDBUILD/Tes/MuFilterEff_run'+str(options.runNumber)+'.root')
