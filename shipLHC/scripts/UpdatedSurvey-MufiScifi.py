...
def Internship():
    ...
    if print_type == 1:
        ...
        if not exists('EventsData.csv'):
            wHeader = 1
        else:
            wHeader = 0
        with open('EventsData.csv', 'a', encoding='UTF8') as f:
            writer = csv.writer(f)
            if wHeader == 1:
                writer.writerow(["Total", "Correct Area", "Muon Gen (in Area)",  "Muon Det (in Area)", "Eff - Muon Det if when Gen (in Area)", "False Positive - Muon Not Gen when Det (In Area)"])
            writer.writerow([WEvents, WCorrectArea, WMuonGen, WMuonDet, WEfficiency, WFalsePositive])
