...
parser.add_argument("-i", "--initialGST", dest="initialGST", help="Get first event ID", default=0)

options = parser.parse_args()

initialGST = options.initialGST
...
'''
    ...
    eventChain.Add("/afs/cern.ch/user/b/bcaetano/private/SND/sndsw/shipLHC/SimResults/Neutrino_Muon_up_14TeV_18576/00200/sndLHC.Genie-TGeant4_dig.root")
...
for i in range(n_files_to_read):
    ...
    if not exists(file_name):
        ...
        continue
    this_read = eventChain.Add(file_name)
...
def GetBadParticles():
    ...
    if eventTree:
        MuFilterHits = eventTree.Digi_MuFilterHits
        ScifiHits       = eventTree.Digi_ScifiHits
        EventHeader        = eventTree.EventHeader
    else:
    # Try the FairRoot way 
        MuFilterHits = ioman.GetObject("Digi_MuFilterHits")
        ScifiHits = ioman.GetObject("Digi_ScifiHits")
        EventHeader = ioman.GetObject("EventHeader")

        # If that doesn't work, try using standard ROOT
        if MuFilterHits == None :
            if logger.IsLogNeeded(ROOT.fair.Severity.info):
                print("Digi_MuFilterHits not in branch list")
            MuFilterHits = ioman.GetInTree().Digi_MuFilterHits
        if ScifiHits == None :
            if logger.IsLogNeeded(ROOT.fair.Severity.info):
                print("Digi_ScifiHits not in branch list")
            ScifiHits = ioman.GetInTree().Digi_ScifiHits
        if EventHeader == None :
            if logger.IsLogNeeded(ROOT.fair.Severity.info):
                print("EventHeader not in branch list")
            EventHeader = ioman.GetInTree().EventHeader
    ...
    eventNList = {27} #Events We want to check
    # zmin = 492.19
    # zmax = 494.09
    zmin = 358.4387
    ...
    for eventID, event in enumerate(eventTree):
        if eventID in eventNList:
            if event.MCTrack[0].GetPdgCode() == 14 or event.MCTrack[0].GetPdgCode() == -14:
                if event.MCTrack[0].GetStartX() >= xmin and event.MCTrack[0].GetStartX() <= xmax and event.MCTrack[0].GetStartY() >= ymin and event.MCTrack[0].GetStartY() <= ymax: #Check if in correct area
                    Nev = 999999
                    withReco = True
                    optionTrack='DS'
                    ...
                    if  Reco_MuonTracks.GetEntries()==1:
                        ...
                        print(f"\n\nTrack:  {theTrack.getPointWithMeasurement()}\n\n")         
    ...
    #             # MuFilterHits = eventTree.Digi_MuFilterHits
    #             # #print(f"\n\n\n{MuFilterHits[1].Print()}\n\n\n")
    #             #for hitID, hit in enumerate(MuFilterHits):
    #             #    print(f"\n\nHIT: {hit.Print()}\n\n")
    #             #print(f"\n\n{event.point()}")
    #             #for particleID, particle in enumerate(event.MCTrack):
    #                 #print(f"\nParticle: {particleID};      {particle.GetPdgCode()}")
    #             #    if particle.GetStartZ() >= zmin and particle.GetStartZ() <= zmax:
    #             #         print(f"\n\nHit: {particle.GetStartX()}, {particle.GetStartY()}, {particle.GetStartZ()}")
    #             #         motherIDlist.append(particle.GetMotherId())
    #             #         #print("--------------------------------------------------------------------------------------------------------------")
...
def Mufi_Efficiency(Nev=options.nEvents,optionTrack=options.trackType,c0=options.refChannel,withReco='True',NbinsRes=100,X=10.,correctTime=options.correctTime):
 projs = {1:'Y',0:'X'}
 correctRun = 46
...
# QDC/TDC channel variations
                  ...
                  for i in allTDCs:
                      ...
                      if  i < nSiPMs:  # left side
                          ...
                          nR+=1
                          meanR+=allTDCs[i]
                  for i in allTDCs: ################# Need to alter from here on out ####################
                     if s==2 and smallSiPMchannel(i): continue
 ...
 #ut.writeHists(h,'/afs/cern.ch/user/g/gmachadosnd/private/SNDBUILD/TDC_Histos/MuFilterEff_run'+str(options.runNumber)+'.root')
 ut.writeHists(h,'/afs/cern.ch/user/b/bcaetano/private/SND/sndsw/shipLHC/scripts/DataDump/MuFilterEff_run'+str(options.runNumber)+'.root')
